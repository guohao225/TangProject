...
class CRF(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        if self.use_kernel:
            self._dense_layer = tf.keras.layers.Dense(
                units=self.units, dtype=self.dtype, kernel_initializer=dense_initializer
            )
        else:
    ...
    def loss(self, y_true, y_pre):
        # loss = self.compute_dsc_loss(y_true, y_pre)
        sequence_length = tf.reduce_sum(tf.cast(y_true != 0, tf.int32), axis=1)
        log_likelihood, _ = crf_log_likelihood(
            y_pre, y_true, sequence_length, self.chain_kernel)
        return -tf.reduce_mean(log_likelihood)

    @tf.function
    def compute_dsc_loss(self, y_true, y_pred, alpha=0.6):
